...
require_once 'C:\xampp\htdocs\islem\projetweb\includes\function.php';
require_once 'C:\xampp\htdocs\islem\projetweb\includes\classe.php'; 
require_once 'C:\xampp\htdocs\islem\projetweb\includes\functionpaiments.php';
...
if (isset($_GET['song_id']) && isset($_GET['type'])) {
    ...
    if($type ='paiments'){
        $deleteQuery = "DELETE FROM transactions WHERE ID = :song_id";
        $deleteStmt = $pdo->prepare($deleteQuery);
        $deleteStmt->bindParam(':song_id', $songId, PDO::PARAM_INT);
        if ($deleteStmt->execute()) {
            $_SESSION['message'] = "Chanson supprimée avec succès !";
