...
require_once 'C:\xampp\htdocs\projetweb\controlleur\function_2.php'; // Make sure this includes the necessary functions
...
if (isset($_POST['shared_link'])) {
    ...
    if (isset($queryParams['id']) && isset($queryParams['user_id'])) {
        ...
        if ($stmt->rowCount() > 0) {
            $_SESSION['message'] = "Playlist already exists!";
            header("Location: avec_connexion.php");
            exit();
        } else {
            // Fetch the data of the playlist
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $createStmt->bindParam(':userId', $iduser, PDO::PARAM_INT);
                ...
                $_SESSION['message'] = "New Playlist with Songs added!";
                header("Location: avec_connexion.php");
                exit();
            }
        }
